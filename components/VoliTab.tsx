...
export default function VoliTab({ isAdmin }: { isAdmin: boolean }) {
    ...
    // Corregge il formato data per i browser mobile
    const formattaDataISO = (str: string) => str ? str.replace(' ', 'T') : '';
    ...
    const handleAggiungiVolo = async () => {
        ...
        try {
            ...
            if (!live) throw new Error("Dati non disponibili per questo volo/data.");
            ...
            await supabase.from('voli').insert([{
                ...
                orario_partenza: formattaDataISO(live.departure?.scheduledTime?.local),
                orario_arrivo: formattaDataISO(live.arrival?.scheduledTime?.local),
        ...
        <div className="space-y-6">
            ...
            {isVoloFormOpen && (
                <div className="bg-white p-6 rounded-3xl border-2 border-blue-500 shadow-xl space-y-4">
                    <input type="text" placeholder="Codice (es: HV5466)" className="w-full p-4 bg-slate-50 rounded-xl border outline-none" value={nuovoVolo.codice} onChange={e => setNuovoVolo({...nuovoVolo, codice: e.target.value})} />
                    ...
                    <input type="text" placeholder="Gruppo (es: Da Milano)" className="w-full p-4 bg-slate-50 rounded-xl border outline-none" value={nuovoVolo.gruppo} onChange={e => setNuovoVolo({...nuovoVolo, gruppo: e.target.value})} />
                    <button onClick={handleAggiungiVolo} disabled={isVerifying} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold">
                        {isVerifying ? "Verifica in corso..." : "Verifica e Attiva Monitoraggio 🚀"}
                    </button>
                </div>
            )}
            ...
            {voli.map(v => {
                ...
                const dLive = formattaDataISO(api?.departure?.actualTime?.local || api?.departure?.predictedTime?.local);
                const hasDelay = dLive && Math.abs(new Date(dLive).getTime() - dSched.getTime()) > 60000;

                return (
                    <div key={v.id} onClick={() => setSelectedVolo(v)} className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-200 relative overflow-hidden cursor-pointer">
                        <div className="absolute top-0 right-0 bg-blue-600 text-white px-4 py-1.5 rounded-bl-2xl text-[10px] font-black uppercase">{v.gruppo || 'Missione'}</div>
                        
                        <p className="text-[10px] font-black text-blue-500 uppercase tracking-tighter mb-4">
                            {dSched.toLocaleDateString('it-IT', { day: '2-digit', month: 'short', year: 'numeric' })}
                        </p>
                        ...
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex-1 flex flex-col items-center px-4">
                                ...
                                <p className="text-[9px] font-black text-blue-600 mt-2">{v.codice_volo}</p>
                            </div>
                            <div className="text-center">
                                <p className="text-4xl font-black tracking-tighter text-slate-800">{v.arrivo_aeroporto}</p>
                                <p className="text-sm font-bold text-slate-600">{aSched.toLocaleTimeString('it-IT', {hour:'2-digit', minute:'2-digit'})}</p>
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6" onClick={() => setSelectedVolo(null)}>
                    <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                <h3 className="text-2xl font-black italic">Logistica Volo</h3>
                        ...
                        <div className="space-y-4">
                            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex justify-between">
                                <div><p className="text-[10px] font-black text-slate-400 uppercase">Aereo</p><p className="font-bold text-slate-700">{selectedVolo.last_api_response?.aircraft?.model || "Dati non disp."}</p></div>
                        ...
                        {isAdmin && (
                            <button onClick={async () => { if(confirm("Eliminare?")) { await supabase.from('voli').delete().eq('id', selectedVolo.id); setSelectedVolo(null); scaricaVoli(); } }} className="w-full mt-6 text-red-400 font-bold text-xs uppercase flex items-center justify-center gap-1"><Trash2 size={14}/> Elimina Volo</button>
                        )}
                    </div>
                </div>
            )}
